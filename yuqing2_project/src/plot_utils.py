...
class PlotUtils:
    ...
    def plot_multi_pie_line(p):
        ...
        for s in sentiments:
            values = []
            for k in keys:
                values.append(len(df[(df['信息来源'] == k) & (df[p.sentiment_col] == s)]))
            c.add_yaxis(s, values)
        c.set_global_opts(title_opts=opts.TitleOpts(title='情感折线图'), xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(rotate=-75)))
        return c