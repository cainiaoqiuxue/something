...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
          ...
          geocode=None, minreplies=None, minlikes=None, minretweets=None, cookie_file=None, token=None, sleep_interval=1.5):
    """
    ...
    if save_images == True:
        show_images = True
    # initiate the driver
    driver = init_driver(headless, proxy, show_images, cookie_file=cookie_file, token=token)
    ...
    data = pd.DataFrame(data, columns = ['UserScreenName', 'UserName', 'Timestamp', 'Text', 'Embedded_text', 'Emojis', 
                              'Comments', 'Likes', 'Retweets','Image link', 'Tweet URL', 'Language'])
