...
class HongYiSpider(BaseSpider):
    ...
    def get_url(self, keyword):
        url = "https://www.so.com/?src=360portal"
        self.driver.get(url)
        inputs = self.driver.find_element(By.ID, 'input')
        inputs.send_keys(keyword)
        inputs.send_keys(Keys.ENTER)
    
    def search(self, keyword, page):
        print('360 search: {}-{}'.format(keyword, page))
        if page == 1:
        ...
        for c in contents:
            try:
                ...
                pass
        self.add_to_json(result, self.save_dir / ("360_{}.json".format(keyword)))

    def get_next_page(self):
        button = self.driver.find_element(By.ID, 'snext')
        button.click()
