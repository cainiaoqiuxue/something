...
class BgNbd:
    ...
    def __init__(self, frequency, recency, period, init_params=None):
        ...
        # params = [r, alpha, a, b]
        self.params = np.array(init_params) if init_params is not None else np.ones(4) * 0.1
        self.predict = self.calculate_conditional_expectation
    ...
    def negative_log_likelihood(self, params):
        ...
        ln_A_4 = self.x.copy()
        ln_A_4[ln_A_4 > 0] = (np.log(a) - np.log(b + ln_A_4[ln_A_4 > 0] - 1) - (r + ln_A_4[ln_A_4 > 0]) * np.log(
            alpha + self.scale_tx))
        delta = np.where(self.x > 0, 1, 0)
        log_likelihood = ln_A_1 + ln_A_2 + np.log(np.exp(ln_A_3) + delta * np.exp(ln_A_4))
        res = -log_likelihood.sum()
        return res
    ...
    def negative_log_likelihood_v2(self, log_params):
        ...
        ll = weights * (A_1 + A_2 + np.log(
            np.exp(A_3 - max_A_3_A_4) + np.exp(A_4 - max_A_3_A_4) * (self.x > 0)) + max_A_3_A_4)
...
class NewBgNbd:
    def __init__(self, frequency, recency, period, add_value, value_type, init_params=None):
        self.scale = 1 / period.max()
        self.x = frequency
        self.t_x = recency
        self.T = period
        ...
        self.add_size = len(add_value)
        self.values = self.add_value.mean()
        self.value_type = value_type  # only {0: buy, 1: lost, 2: all}
        self.predict = self.calculate_conditional_expectation
        if init_params is not None:
            self.params = np.array(init_params)
        else:
            self.params = np.ones(4 + self.value_type + 1) * 0.1

    def negative_log_likelihood(self, params):
        if np.any(np.asarray(params)) <= 0:
            return np.inf
        if self.value_type == 0:
            r, alpha, a, b, gamma_1 = params
            gamma_2, gamma_3 = 0, 0
        elif self.value_type == 1:
            r, alpha, a, b, gamma_2, gamma_3 = params
            gamma_1 = 0
        else:
            r, alpha, a, b, gamma_1, gamma_2, gamma_3 = params
        values = self.values
        B_1 = gammaln(r + self.x) - gammaln(r) + r * np.log(alpha * np.exp(-gamma_1 * values))
        B_2 = gammaln(a * np.exp(gamma_2 * values) + b * np.exp(gamma_3 * values)) + gammaln(
            b * np.exp(gamma_3 * values) + self.x) - gammaln(b * np.exp(gamma_3 * values)) - gammaln(
            a * np.exp(gamma_2 * values) + b * np.exp(gamma_3 * values) + self.x)
        B_3 = -(r + self.x) * np.log(alpha * np.exp(-gamma_1 * values) + self.scale_T)
        B_4 = self.x.copy()
        B_4[B_4 > 0] = np.log(a) + gamma_2 * values - np.log(
            b * np.exp(gamma_3 * values) + B_4[B_4 > 0] - 1) - (r + B_4[B_4 > 0]) * np.log(
            alpha * np.exp(-gamma_1 * values) + self.scale_tx)
        delta = np.where(self.x > 0, 1, 0)
        log_likelihood = B_1 + B_2 + np.log(np.exp(B_3) + delta * np.exp(B_4))
        res = -log_likelihood.sum()
        return res
    ...
    def fit(self, **kwargs):
        ...
        self.params = np.array([output.x[i] for i in range(len(self.params))])
        self.params[1] /= self.scale

    def expected_sales_to_time_t(self, t):
        if self.value_type == 0:
            r, alpha, a, b, gamma_1 = self.params
            gamma_2, gamma_3 = 0, 0
        elif self.value_type == 1:
            r, alpha, a, b, gamma_2, gamma_3 = self.params
            gamma_1 = 0
        else:
            r, alpha, a, b, gamma_1, gamma_2, gamma_3 = self.params

        alpha = alpha * np.exp(-gamma_1 * self.values)
        a = a * np.exp(gamma_2 * self.values)
        b = b * np.exp(gamma_3 * self.values)
    ...
    def calculate_conditional_expectation(self, t, x, t_x, period):
        ...
        if isinstance(x, pd.Series):
            ...
            delta = 1 if x > 0 else 0
        denominator = 1 + delta * (a / (b + x - 1)) * ((alpha + period) / (alpha + t_x)) ** (r + x)
        penalize = t ** 2 * (self.value_type + 1) * 5e-4
        return first_term * second_term / denominator - penalize
